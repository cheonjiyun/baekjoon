...
public class Main {
  ...
  public static void explore(int cctvIdx, int canViewArea) {
    if (cctvIdx == cctv.size()) { // 모든 cctv를 다 돌았으면 멈춰
      if (canViewArea > maxArea) // 최대로 감시할 수 있는
        maxArea = canViewArea;
      return;
    }
    ...
    for (int i = 0; i < 4; i++) {
      ...
      List<XY> changeXY = new ArrayList<>(); // 바꾼 값 기록하고, 90도 회전할 때마다 초기화 할 예정

      // CCTV가 볼 수 있는 모든 방향
      for (int direction : directions) {
        int realDirection = (direction + i) % 4; // 90 도 회전 값

        // 한 방향이 볼 수 있는 일직선 뻗어나감
        int nx = dx[realDirection] + x;
        int ny = dy[realDirection] + y;

        while (nx >= 0 && ny >= 0 && nx < m && ny < n && office[ny][nx] != WALL) { // 벽, 벗어나느것 말고 다
          if (!(office[ny][nx] >= 1 && office[ny][nx] <= 5) && office[ny][nx] != 7) { // cctv가 아니고,
                                                                                      // 이미 볼 수 있는
                                                                                      // 곳이
                                                                                      // 아니라면
            office[ny][nx] = CCTVVIEW; // 보는 지역이라고 기록
